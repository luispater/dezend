...
ULONG StackRemove( PCOMMON_STACK Stack, PVOID Element )
{
    ...
    while ( Iter < Count )
    {
        if ( Stack->Elements[Iter] == Element )
        {
            HEAP_FREE( Stack->Elements[Iter] );
            Length = Count - Iter - 1;
            memcpy( Stack->Elements + Iter, Stack->Elements + Iter + 1, Length );
            RetCode = --Stack->Top;
            break;
        }
        Iter++;
...
void StackApply( PCOMMON_STACK Stack, ULONG Type, STACK_FUNCTION ApplyFunction )
{
    ...
    if ( Type == ZEND_STACK_APPLY_TOPDOWN )
    {
    ...
    else if ( Type == ZEND_STACK_APPLY_BOTTOMUP )
    {
...
__inline ULONG StackAddElementT( PCOMMON_STACK_T Stack, PVOID Element, ULONG Size )
{
    if ( Stack->Top >= Stack->Max )
    {
...
void StackSetSortCompareT( PCOMMON_STACK_T Stack, COMPARE_FUNCTION Compare )
{
    if ( Compare == NULL )
...
void StackSetFindCompareT( PCOMMON_STACK_T Stack, COMPARE_FUNCTION Compare )
{
    if ( Compare == NULL )
...
ULONG StackRemoveT( PCOMMON_STACK_T Stack, PVOID Element )
{
    ...
    while ( Iter < Count )
    {
        ...
        {
            Length = Count - Iter - 1;
            memcpy( Stack->Elements + Iter, Stack->Elements + Iter + 1, Length * sizeof( Stack->Elements[0] ) );
            RetCode = --Stack->Top;
            break;
        }
        Iter++;
